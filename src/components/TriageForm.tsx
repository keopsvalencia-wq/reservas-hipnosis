                            ...
                            <div className="flex flex-col gap-0.5 md:gap-4">
                                {q.options?.map(opt => (
                                    <button
                                        ...
                                        type="button"
                                        onClick={() => handleAnswer(q.id, opt.value)}
                                        className={`w-full flex items-center text-left gap-1.5 md:gap-5 p-0.5 md:p-5 rounded-lg border-2 transition-all group ${answers[q.id] === opt.value
                                            ? 'border-[var(--color-primary)] bg-[var(--color-primary-soft)]'
                                            : 'border-gray-50 hover:border-[var(--color-primary)] bg-white shadow-sm'
                                            }`}
                                    >
                                        <div className={`h-3 w-3 md:h-6 md:w-6 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-colors ${answers[q.id] === opt.value ? 'border-[var(--color-primary)] bg-white' : 'border-gray-300'}`}>
                                            {answers[q.id] === opt.value && <div className="h-1.5 w-1.5 md:h-3.5 md:w-3.5 rounded-full bg-[var(--color-primary)]" />}
                                        </div>
                                        <span className={`text-[10px] md:text-xl font-bold transition-colors leading-tight ${answers[q.id] === opt.value ? 'text-[var(--color-secondary)]' : 'text-[var(--color-text-muted)] group-hover:text-[var(--color-text)]'}`}>
                                            {opt.label}
                                        </span>
                                    </button>
                                ))}
